...
load('lbp_coral_feature.mat', 'lbp_f1');

num_images = size(lbp_f1, 1);
distances = zeros(num_images, num_images);

for i = 1:num_images
    for j = 1:num_images
        distances(i, j) = sqrt(sum((lbp_f1(i, :) - lbp_f1(j, :)).^2));
    end
end
% for i = 1:num_images
%     for j = 1:num_images
%         distances(i, j) = ceil(distances(i, j));
%     end
% end
...
save('Average_PRRR.mat','avg_pre','avg_rec');
